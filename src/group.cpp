...
void Group::MenuGroup(Command id) {
	...
	switch(id) {
		...
		case Command::GROUP_MIRROR : {
			bool ok = false;
			if (gs.n == 1) {
				if ((gs.lineSegments == 1) && SS.GW.LockedInWorkplane()) {
					// Mirror across workplane orthogonal to line segment in 2D view
					g.predef.entityB = SS.GW.ActiveWorkplane();
					ok = true;
				}
				else if (gs.workplanes == 1) {
					// Mirror across selected workplane in 2D or 3D view
					g.predef.entityB = gs.entity[0];
					ok = true;
				}
			}
			if (ok) {
				...
				Error(_("Bad selection for new mirror. This group can "
						"be created with:\n\n"
						"	 * a line segment (mirror across line)\n"
						"	 * a workplane (mirror across workplane)\n"));
				return;
			}
		}
